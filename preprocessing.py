...
class SeaLionData(object):
    def __init__(self, sourcedir=SOURCEDIR, outdir=OUTDIR, verbosity=VERBOSITY.NORMAL):
        ...
        self.paths = {
            ...
            'chunk': os.path.join(outdir, 'chunks/{size}/{tid}_{cls}_{row}_{col}_{size}.png'),
        }
...
if __name__ == "__main__":
    ...
    sourcedir = '../data/small/'
    outdir = '../data/small/output/'
    verbosity = VERBOSITY.VERBOSE

    sld = SeaLionData(sourcedir=sourcedir, outdir=outdir, verbosity=verbosity)

    tids = [41, 42] #sld.trainshort2_ids
    sld.save_coords(tids)

    for tid in tids:
        sld.save_sea_lions(tid)